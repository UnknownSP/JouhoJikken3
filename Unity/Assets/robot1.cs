...
public class robot1 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        robot_3.transform.position = new Vector3(-4.0f, 0.0f, 0.0f);
    ...
    private static float r1_stop_time = 8.0f + r1_straight_time;
    private static float r3_straight_time = 1.2f;
    private static float r3_stop_time = 6.8f + r3_straight_time;
    private static float r3_right_white_time = 4.5f + r3_straight_time;
    private static float r3_left_white_time = 6.7f + r3_straight_time;
    int count = 0;
    ...
    void Update()
    {
        ...
        {
            robot_3.transform.position = new Vector3(-4.0f + now_time * 1.6f, 0.0f, 0.0f);
        }
        else if (now_time <= r3_stop_time)
        {
           
            float x_3 = (now_time - r3_straight_time) * 0.2f - 4.0f + r3_straight_time * 1.6f + 5.0f * Mathf.Sin(2.0f * Mathf.PI * (now_time - r3_straight_time) * 0.04f);
            /*if (now_time >= 6.0)
            {
                x_3 += (now_time - 6.0f)*0.2f;
            }*/
            float z_3 = 5.0f * (1.0f - Mathf.Cos(2.0f * Mathf.PI * (now_time - r3_straight_time) * 0.038f));
            robot_3.transform.position = new Vector3(x_3, 0.0f, z_3);
            robot_3.transform.localRotation = Quaternion.Euler(0.0f, -(now_time - r3_straight_time) * 12.0f, 0.0f);
