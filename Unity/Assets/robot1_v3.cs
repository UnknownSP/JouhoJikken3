...
public class robot1_v3 : MonoBehaviour
{
    ...
    void Start()
    {
        robot_1 = GameObject.Find("robot2");
        robot_1.transform.position = new Vector3(0.0f, 0.0f, 0.0f);
        robot_3 = GameObject.Find("robot3");
        robot_3.transform.position = new Vector3(-3.0f, 0.0f, -0.1f);
        robot_3.transform.localRotation = Quaternion.Euler(0.0f, 9.0f, 0.0f);
    ...
    private static float r1_stop_time = 10.0f + r1_straight_time;
    private static float r3_straight_time = 1.0f;
    private static float r3_stop_time = 9.0f + r3_straight_time;
    ...
    void Update()
    {
        camera.transform.position = (robot_1.transform.position + robot_3.transform.position)/2.0f;
        camera.transform.position += new Vector3(0.0f, 2.0f, 0.0f);
        //camera.transform.position += new Vector3(0.0f, 1.0f, 1.0f);
        count += 1;
        ...
        {
            robot_3.transform.position = new Vector3(-3.0f + now_time * 1.5f, 0.0f, -0.1f - now_time * 0.2f);
        }
        else if (now_time <= r3_stop_time)
        {
           
            float x_3 = /*(now_time - r3_straight_time) * 0.15f*/ - 3.0f + r3_straight_time * 1.5f + 5.0f * Mathf.Sin(2.0f * Mathf.PI * (now_time - r3_straight_time) * 0.039f);
            float z_3 = -0.1f - r3_straight_time*0.2f + 5.0f * (1.0f - Mathf.Cos(2.0f * Mathf.PI * (now_time - r3_straight_time) * 0.042f));
            robot_3.transform.position = new Vector3(x_3, 0.0f, z_3);
            float w = 9.0f - (now_time - r3_straight_time) * 15.5f;
            if (now_time >= 8.0)
            {
                w -= (now_time - 8.0f) * 2.0f;
            }
            robot_3.transform.localRotation = Quaternion.Euler(0.0f, w, 0.0f);
